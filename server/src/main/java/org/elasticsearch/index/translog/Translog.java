...
public class Translog extends AbstractIndexShardComponent implements IndexShardComponent, Closeable {
    ...
    public static class Index implements Operation {
        ...
        /** Version when source xcontent type was added to operation stored in translog **/
        public static final Version VERSION_SOURCE_CONTENT_TYPE_ADDED = Version.V_8_6_0;
        ...
        public static final int FORMAT_WITH_SOURCE_CONTENT_TYPE = FORMAT_NO_DOC_TYPE + 1; // since VERSION_SOURCE_CONTENT_TYPE_ADDED
        public static final int SERIALIZATION_FORMAT = FORMAT_WITH_SOURCE_CONTENT_TYPE;
        ...
        private Index(final StreamInput in) throws IOException {
            ...
            if (format < FORMAT_WITH_SOURCE_CONTENT_TYPE) {
                sourceContentType = (source.length() == 0) ? XContentType.JSON : XContentHelper.xContentType(source);
            } else {
                sourceContentType = in.readEnum(XContentType.class);
            }
            assert invariant();
        }
        ...
        public Index(Engine.Index index, Engine.IndexResult indexResult) {
            ...
            this.sourceContentType = requireCanonical(index.sourceContentType());
        ...
        public Index(
            ...
            this.source = source;
            this.sourceContentType = requireCanonical(sourceContentType);
        ...
        public long estimateSize() {
            long estimateSize = 2L * id.length();
            estimateSize += source.length();
            estimateSize += Byte.BYTES; // source's xcontent type has 4 canonical values encoded as variable-length integer, so 1 byte
            if (routing != null) {
                estimateSize += 2L * routing.length();
            }
            estimateSize += 4L * Long.BYTES; // timestamp, seq_no, primary_term, and version
            return estimateSize;
        }
        ...
        private void write(final StreamOutput out) throws IOException {
            final int format;
            if (out.getVersion().onOrAfter(VERSION_SOURCE_CONTENT_TYPE_ADDED)) {
                format = SERIALIZATION_FORMAT;
            } else if (out.getVersion().onOrAfter(Version.V_8_0_0)) {
        ...
        public boolean equals(Object o) {
            ...
            if (version != index.version
                ...
                || sourceContentType != index.sourceContentType
        ...
        public int hashCode() {
            ...
            result = 31 * result + sourceContentType.hashCode();
        ...
        private boolean invariant() {
            ...
            assert sourceContentType == sourceContentType.canonical();
            XContentType detectedContentType = XContentHelper.xContentType(source);
            assert sourceContentType == detectedContentType || source.length() == 0 : sourceContentType + " vs " + detectedContentType;
            return true;
        }

        private static XContentType requireCanonical(XContentType type) {
            return Objects.requireNonNull(type, "XContentType is null").canonical();
        }
    }
    ...
    public static class Delete implements Operation {
        ...
        @Override
        public XContentType sourceContentType() {
            throw new IllegalStateException("trying to read doc source xcontent type from delete operation");
        }
    ...
    public static class NoOp implements Operation {
        ...
        @Override
        public XContentType sourceContentType() {
            throw new UnsupportedOperationException("source xcontent type does not exist for a no-op");
        }
