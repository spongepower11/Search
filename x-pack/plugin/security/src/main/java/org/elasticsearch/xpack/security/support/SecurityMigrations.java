...
public class SecurityMigrations {
    private static final Logger LOGGER = LogManager.getLogger(SecurityMigrations.class);
    ...
    public static final Integer ROLE_NAME_MIGRATION_VERSION = 2;
    ...
    public static final TreeMap<Integer, SecurityMigration> MIGRATIONS_BY_VERSION = new TreeMap<>(
        Map.of(ROLE_METADATA_FLATTENED_MIGRATION_VERSION, new SecurityMigration() {
        ...
        }, ROLE_NAME_MIGRATION_VERSION, new SecurityMigration() {
            ...
            private void updateRolesByQuery(
                ...
                updateByQueryRequest.setQuery(filterQuery);
                updateByQueryRequest.setScript(
                    new Script(ScriptType.INLINE, "painless", "ctx._source.name = ctx._id.substring(5)", Collections.emptyMap())
                );
                client.admin()
                    .cluster()
                    .execute(UpdateByQueryAction.INSTANCE, updateByQueryRequest, ActionListener.wrap(bulkByScrollResponse -> {
                        LOGGER.info(
                            "Migration [" + ROLE_NAME_MIGRATION_VERSION + "] migrated [" + bulkByScrollResponse.getTotal() + "] roles"
                        );
                        listener.onResponse(null);
                    }, listener::onFailure));
            }
            ...
            @Override
            public Set<NodeFeature> nodeFeaturesRequired() {
                return Set.of(SecurityFeatures.SECURITY_ROLES_INDEX_NAME);
            }
